...
var ClientSideWebPartManager =  (function () {
    ...
    ClientSideWebPartManager.prototype._onWebPartRendered = function (context, renderEndTime) {
        ...
        if (ViewportLoader.optimizeViewportLoader() && trackingCallback) {
    ...
    ClientSideWebPartManager.prototype._onAllWebpartsAdded = function (context, startTime) {
        ...
        else if (!ViewportLoader.optimizeViewportLoader()) {
    ...
    ClientSideWebPartManager.prototype._getViewportRenderedEndTimeLegacy = function () {
        ...
        this._webparts.forEach(function (wpi) {
            if (_this._instanceOfBase(wpi)
                ...
                && ViewportLoader.isInInitialViewport(wpi)) {
    ...
    ClientSideWebPartManager.prototype._getViewportRenderedEndTime = function () {
        ...
        var breakLoop = false;
        this._webparts.forEach(function (wpi) {
            if (!breakLoop && _this._instanceOfBase(wpi) && !_PerformanceLogger.shouldSkipComponent(wpi.context.webPartTag)) {
    ...
    ClientSideWebPartManager.prototype._registerWebpartsInViewport = function () {
        ...
        this._webparts.forEach(function (wpi) {
            if (_this._instanceOfBase(wpi)) {
                ...
                if (ViewportLoader.isInInitialViewport(wpi)) {
    ...
    ClientSideWebPartManager.prototype._onFirstTimeWebpartRender = function (manifest,  webPartTag, qosMonitor, context) {
        if (!KillSwitches.isWebPartLoadDataCollectorKSActivated()) {
            WebPartLoadDataCollector.collect(manifest, webPartTag, qosMonitor, this._pageContext);
        }
        else {
            ...
            var isSpinnyShown = !!_PerformanceLogger.readComponentBreakdown(webPartTag, 'displaySpinner');
            var cacheHit = _PerformanceLogger.readComponentBreakdown(webPartTag, 'CacheHit');
            var cacheMiss = _PerformanceLogger.readComponentBreakdown(webPartTag, 'CacheMiss');
            ...
            var extraData = this._createSuccessExtraData(manifest.alias, manifest.isInternal, manifest.id, moduleLoaded - start, initialized - inViewportLoaded, end - initialized, isSpinnyShown, _PerformanceLogger.getScenarioId(), isMultiGeo, viewportWait, webPartCacheHit);
    ...
    ClientSideWebPartManager.prototype._renderWebPart = function (options) {
        ...
        try {
            ...
            return this._loadPropertyPaneModule().then(function () {
                ...
                if ((Environment.type === EnvironmentType.ClassicSharePoint) && ViewportLoader.isInInitialViewport(wpi_1)) {
