...
- Upgrade @types/lodash
- Add new web part lifecycle event for taking custom actions when data is updated by sources out of web part or property pane
- Upgrate TypeScript to 3.3
- Bump up TypeScript to 2.9.2
- api extractor config changes
- Move the microsoftTeams property to public
- clean up KillSwitches
- Upgrade react to 16.7
- Only killswitch.
- adding max-height constraint to the loading indicator
- adding viewport delay timeout for the loading indicator
- Adding perf dimensions for layout and dataprovider if available 
- Adding a parameter to make sure that webpard spinny data is logged
- Refactoring log collection into separate class
...
- Fixing an issue where isolated web parts are not loading on the app pages
...
- Created IViewportAwareComponent interface and extand extended IViewportAwareWebPart from this new interface. Renamed properties within each and updated descriptions.
