...
class DevDeploy {
    constructor(options) {
        this._options = Object.assign({ useRush: false, isolateProjects: true, ignoreCollisions: false }, options);
    ...
    execute() {
                    ...
                    if (manifests[id].hasOwnProperty(version)) {
                        ...
                        const processPath = (resourcePath) => {
                            ...
                            if (url_1.isUrl(resourcePath) || isBaseUrlRelative) {
                                ...
                                dirsToUpload.set(filesystemDirectory, projectContainerName);
        ...
        dirsToUpload.forEach((deployDirName, localDirName) => {
        ...
        return Promise.all([
            node_core_library_1.LegacyAdapters.convertCallbackToPromise(Webpack, [webpackConfig]).then(() => {
                filesToUpload.push({
                    localPath: manifestsFileFullPath,
                    deployPath: debugManifestPath
                });
                return this._options.deployPathsFunction(filesToUpload);
            }),
            ...directoryExpandPromises
        ]).then(() => {
    ...
    _getManifests(cumulativeManifestProcessor) {
        if (this._options.useRush) {
            ...
            return cumulativeManifestProcessor.discoverManifests(this._options.rootPath, cumulativeManifestProcessor_1.ForceSearch.deepSparse);
        }
    }
