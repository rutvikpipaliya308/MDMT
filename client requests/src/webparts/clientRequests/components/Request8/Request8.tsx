...
export default class Request8 extends React.Component<IRequest8Props, IRequest8State> {
    ...
    public async componentWillMount() {
        ...
        if (hasAddPermission && isAccessLevelExists) {
            ...
            else if (queryParameters.getValue("companyNo")) {
                let companyNo: string = queryParameters.getValue("companyNo");
                if ((companyNo)) {
                    ...
                    tempData.filter((tempItem) => {
                        ...
                        if (tempItem.ClientCompanyNo == companyNo.toString() && isAccessLevelPresent) {
                            tempArray.push(tempItem);
                        }
                    });
                    ...
                    let item = tempArray[0];
                    if (item !== null && item !== undefined) {
                        this.setState({ companyNo: companyNo });
                    }
                    else {
                        this.setState({ dataNotFound: true });
                    }
    ...
    public render(): React.ReactElement<IRequest8Props> {
                        ...
                        <React.Fragment>
                            <div className="container-xl">
                                <ul className="row no-gutters process-block mb-5">
                                    {Constants.REQ8_SECTION_OPTIONS.map((item, i) => (
    ...
    private RenderSections(step: number) {
        ...
        switch (step) {
            case 1:
                return <Section1 {...this.props} itemID={this.state.itemID} isFHDUser={this.state.isFHDUser} companyNo={this.state.companyNo} listData={this.state.listData} data={this.state.section1Data} dataChange={this.dataChange.bind(this)} nextStep={this.NextStep.bind(this)} listOfEditableFields={listOfEditableFields} itemSubmitted={this.state.itemSubmitted} accessLevel={this.state.accessLevel} />;
            case 2:
                return <DueDiligence {...this.props} itemID={this.state.itemID} isFHDUser={this.state.isFHDUser} listData={this.state.listData} data={this.state.dueDiligenceData} dataChange={this.dataChange.bind(this)} nextStep={this.NextStep.bind(this)} backStep={this.BackStep.bind(this)} listOfEditableFields={listOfEditableFields} itemSubmitted={this.state.itemSubmitted} />;
            case 3:
            ...
            case 4:
                return <Section3 {...this.props} itemID={this.state.itemID} isFHDUser={this.state.isFHDUser} listData={this.state.listData} data={this.state.section3Data} dataChange={this.dataChange.bind(this)} section2Data={this.state.DuplicationSectionData} nextStep={this.NextStep.bind(this)} backStep={this.BackStep.bind(this)} contactCompanyNo={this.state.approvalData.contactCompanyNo} listOfEditableFields={listOfEditableFields} itemSubmitted={this.state.itemSubmitted} accessLevel={this.state.accessLevel} CountryOfCompany={this.state.countryOfCompany} />;
            case 5:
                return <Section4 {...this.props} itemID={this.state.itemID} isFHDUser={this.state.isFHDUser} listData={this.state.listData} data={this.state.section4Data} dataChange={this.dataChange.bind(this)} section2Data={this.state.DuplicationSectionData} approvalData={this.state.approvalData} nextStep={this.NextStep.bind(this)} backStep={this.BackStep.bind(this)} listOfEditableFields={listOfEditableFields} itemSubmitted={this.state.itemSubmitted} />;
            case 6:
                return <Attachments {...this.props} itemID={this.state.itemID} listData={this.state.listData} approvalData={this.state.approvalData} backStep={this.BackStep.bind(this)} itemSubmitted={this.state.itemSubmitted} accessLevel={this.state.accessLevel} />;
        }
    }
    ...
    private async getDueDiligenceQuestions() {
        ...
        var data = await this.objWeb.getList(this.props.context.pageContext.web.serverRelativeUrl + Constants.DUEDILQUESTIONSURL).getItemsByCAMLQuery({ ViewXml: viewXML }, 'FieldValuesAsText');
